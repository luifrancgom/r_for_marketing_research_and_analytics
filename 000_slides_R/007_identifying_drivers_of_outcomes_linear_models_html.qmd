...
amusement_park <- amusement_park |>
  mutate(
    weekend = factor(x = weekend, labels = c('no', 'yes'), ordered = FALSE),
...
abline(reg = lm(formula = overall ~ rides, data = amusement_park), col = 'blue')
```
...
amusement_park |>
  ...
  labs(x = "Satisfaction with Rides", y = "Overall Satisfaction")
```
...
augment(x = toy_model) |>
  ggplot(aes(x = rides, y = overall)) +
  geom_point(shape = 21, color = 'black', fill = umng_palette[4], size = 2) +
  geom_smooth(method = 'lm', color = umng_palette[1], se = FALSE) +
  geom_segment(aes(xend = rides, yend = .fitted), color = umng_palette[2]) +
  labs(x = "Satisfaction with Rides", y = "Overall Satisfaction")
```
