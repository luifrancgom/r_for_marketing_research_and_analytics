...
axis(side = 1, at = seq(from = 60, to = 300, by = 20))
lines(
  x = density(weekly_store$p1sales, bw = 10),
  type = "l",
  col = "darkred",
  lwd = 2
)
...
```{r}
boxplot(
  weekly_store$p2sales ~ weekly_store$p2prom,
...
```{r}
#| fig-height: 4
weekly_store |> ggplot() + geom_boxplot(aes(x = p2sales, y = p2prom))
```
...
```{r}
g_labels <- weekly_store_sales_by_country |>
  ggplot() +
  geom_col(
    aes(x = country, y = sum_p2sales, fill = country),
    color = "black",
...
```{r}
g_labels_labs <- weekly_store_sales_by_country |>
  ggplot() +
  geom_col(
    aes(x = country, y = sum_p2sales, fill = country),
    color = "black",
  ...
  labs(x = NULL, y = "Sales (Units)", title = "Sales in units per country")
```
