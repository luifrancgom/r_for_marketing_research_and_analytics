...
segmentation |>
  ...
  pivot_wider(id_cols = subscribe, names_from = ownHome, values_from = n)
```
...
library(tidymodels)
segmentation |>
  chisq_test(response = Segment, p = c(1 / 4, 1 / 4, 1 / 4, 1 / 4))
```
...
chi_statistic <- chisq.test(
  table(segmentation$subscribe, segmentation$ownHome),
  correct = FALSE
)
chi_statistic
```
...
```{r}
segmentation |>
  chisq_test(formula = subscribe ~ ownHome, correct = FALSE)
```
...
segmentation |>
  group_by(ownHome) |>
  summarise(mean_income = mean(income), var_income = var(income), n = n())
```
...
segmentation |>
  group_by(Segment) |>
  summarise(mean = mean(income), variance = var(income), n = n())
```
...
ggplot() +
  geom_function(
    fun = df,
    args = list(df1 = 3, df2 = 294, log = FALSE),
    xlim = c(0, 90),
    color = '#2C3E50'
  ) +
  geom_ribbon(
    data = tibble(
      x = seq.int(
        from = qf(p = 0.05, df1 = 3, df2 = 294, lower.tail = FALSE),
        to = 90,
        by = 0.01
      ),
  ...
  geom_vline(
    xintercept = qf(p = 0.05, df1 = 3, df2 = 294, lower.tail = FALSE),
    color = "#E31A1C"
  ) +
  geom_vline(xintercept = unname(anova_table$`F value`)[1], color = "#18BC9C") +
  scale_x_continuous(
    breaks = c(
      qf(p = 0.05, df1 = 3, df2 = 294, lower.tail = FALSE),
