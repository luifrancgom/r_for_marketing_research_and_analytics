...
```{r}
aggregate(x = income ~ Segment + ownHome, data = segmentation, FUN = mean)
```
...
```{r}
aggregate(x = kids ~ Segment + ownHome, data = segmentation, FUN = sum)
```
...
barchart(
  ...
  data = seg_income_agg,
  groups = ownHome,
  auto.key = TRUE, # Add groups
  par.settings = simpleTheme(col = terrain.colors(n = 2))
...
```{r}
#| fig-height: 4.5
bwplot(Segment ~ income | ownHome, data = segmentation, xlab = 'Income')
```
