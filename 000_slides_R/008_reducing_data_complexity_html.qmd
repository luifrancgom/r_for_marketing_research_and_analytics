...
consumer_brand_scale <- consumer_brand |>
  mutate(across(perform:rebuy, .fns = \(x) {
    scale(x = x, center = TRUE, scale = TRUE)[, 1]
  }))
...
correlation_matrix |>
  autoplot(
    method = "HC", # Hierarchical clustering: More details in Chapter 11
    triangular = "lower"
  )
```
...
brand_mean_longer <- brand_mean |>
  ...
  mutate(
    brand = fct_reorder(.f = brand, .x = value_mean),
    perceptual_adjectives = fct_reorder(
      .f = perceptual_adjectives,
      .x = value_mean
    )
  )
brand_mean_longer
...
consumer_brand_sample |>
  ggplot() +
  geom_vline(xintercept = mean_leader, color = "black") +
  geom_hline(yintercept = mean_perform, color = "black") +
  geom_point(aes(x = leader, y = perform, color = brand)) +
  geom_point(
    data = tibble(
      x = mean(consumer_brand_sample$leader),
...
consumer_brand_sample_center <- consumer_brand_sample |>
  mutate(perform = perform - mean_perform, leader = leader - mean_leader)
...
consumer_brand_sample_center_scale <- consumer_brand_sample_center |>
  mutate(perform = perform / sd_perform, leader = leader / sd_leader)
...
# Fit
v <- prcomp(
  as.matrix(select(consumer_brand_sample_center_scale, -brand)),
  center = TRUE,
  scale. = TRUE
) |>
  _$rotation
...
consumer_brand_sample_center_scale_fit <- consumer_brand_sample_center_scale |>
  mutate(
    perform_end = (beta * leader + perform) / 2,
...
consumer_brand_sample_center_scale_fit |>
  ...
  geom_segment(aes(xend = leader_end, yend = perform_end), color = "#CCBE93") +
  geom_point(
    data = tibble(x = 0, y = 0),
    aes(x = x, y = y),
    color = "#E31A1C"
...
consumer_brand_sample_center_scale_fit <- consumer_brand_sample_center_scale_fit |>
  mutate(
    perform_end2 = ((-beta) * leader + perform) / 2,
...
consumer_brand_sample_center_scale_fit |>
  ...
  geom_segment(aes(xend = leader_end, yend = perform_end), color = "#CCBE93") +
  geom_segment(
    aes(xend = leader_end2, yend = perform_end2),
    color = "#CCBE93"
  ) +
  geom_point(
    data = tibble(x = 0, y = 0),
...
consumer_brand_sample_center_scale_fit |>
  ...
  labs(x = "leader", y = "perform", title = "Proyected points") +
  theme(legend.position = "right", aspect.ratio = 1)
```
...
consumer_brand_sample_center_scale_fit_rotation <- consumer_brand_sample_center_scale_fit |>
  mutate(
    perform_rotation = 0,
    leader_rotation = cos(angle_radians) *
      leader_end +
      sin(angle_radians) * perform_end,
    leader_rotation2 = 0,
    perform_rotation2 = -sin(angle_radians) *
      leader_end2 +
      cos(angle_radians) * perform_end2
  ) |>
  # Possible clusters
  mutate(
    cluster = case_when(
...
consumer_brand_sample_center_scale_fit_rotation |>
  ...
  labs(x = "leader", y = "perform", title = "Rotated points") +
  theme(legend.position = "right", aspect.ratio = 1)
```
...
consumer_brand_sample_center_scale_fit_rotation |>
  ...
  geom_segment(
    aes(
      x = leader_rotation,
      xend = leader_rotation,
      y = 0,
      yend = perform_rotation2
    ),
    color = "#CCBE93"
  ) +
  geom_segment(
  ...
  geom_point(
    aes(x = leader_rotation2, y = perform_rotation2),
    color = "black"
  ) +
...
consumer_brand_sample_center_scale_fit_rotation |>
  ...
  geom_mark_hull(aes(fill = cluster, label = cluster), show.legend = FALSE) +
  geom_vline(xintercept = 0, color = "#18BC9C") +
  geom_hline(yintercept = 0, color = "#18BC9C") +
  geom_point(aes(color = brand)) +
  scale_x_continuous(limits = c(-2, 2)) +
...
consumer_brand_sample_center_scale_fit_rotation_recover <- consumer_brand_sample_center_scale_fit_rotation |>
  mutate(
    perform_recover = (perform_end) * sd_perform + mean_perform,
...
consumer_brand_sample_center_scale_fit_rotation_recover |>
  ...
  geom_point(
    data = consumer_brand_sample,
    aes(x = leader, y = perform, color = "Original data")
  ) +
  scale_x_continuous(breaks = 0:8, limits = c(0, 8)) +
  scale_y_continuous(breaks = 0:11, limits = c(0, 11)) +
  labs(x = "leader", y = "perform", title = "Recovered data vs Original data") +
...
boat_gray_wider <- boat_gray_long |>
  pivot_wider(id_cols = x, names_from = y, values_from = value)
boat_gray_wider
```
...
pca_tidy_reconstruction <- tibble(pc = n_pc) |>
  mutate(
    ...
    x_hat = map(
      .x = x_hat,
      .f = ~ scale(x = .x, center = -boat_gray_wider_pca$center, scale = FALSE)
    ),
    x_hat = map(.x = x_hat, .f = ~ as_tibble(.x)),
    x_hat = map(.x = x_hat, .f = ~ bind_cols(select(boat_gray_wider, x), .x)),
    x_hat = map(
      .x = x_hat,
      .f = ~ pivot_longer(
        .x,
  ...
  mutate(pc_label = str_glue("Number of components: {pc}"), .after = pc) |>
...
pca_tidy_reconstruction |>
  ...
  facet_wrap(facets = vars(pc), nrow = 2, ncol = 3) +
